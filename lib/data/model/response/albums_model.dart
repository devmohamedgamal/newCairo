...
class Albums {
  ...
  factory Albums.fromJson(Map<String, dynamic> json) => Albums(
        ...
        photo: List<List<Photo>>.from(json["photo"]
            .map((x) => List<Photo>.from(x.map((x) => Photo.fromJson(x))))),
      );

  Map<String, dynamic> toJson() => {
        "information": List<dynamic>.from(information!.map((x) => x.toJson())),
        "photo": List<dynamic>.from(
            photo!.map((x) => List<dynamic>.from(x.map((x) => x.toJson())))),
      };
}
